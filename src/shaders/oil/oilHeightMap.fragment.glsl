...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // sample buffer
    vec4 frame = texture2D(iChannel1, uv + strength * offset / iResolution.xy);
    
    // temporal fading buffer
    paint = max(paint, frame.x - iTimeDelta * fade);
    
    // print result
    vec4 testClamp = vec4(vec3(clamp(paint,0.,1.)),1.);
    // fragColor = vec4(clamp(paint, 0., 1.));
    if(shouldFreeze){
        fragColor = vec4(texture2D(iChannel1,uv).rb,1.0,1.0);
    }else{
    fragColor = testClamp;
